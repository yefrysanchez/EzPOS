...
const ProductListMultiForm = () => {
  ...
  return (
    <div className="bg-lightGray/10 relative p-2 text-white text-start rounded-lg h-32 mb-4 grid grid-cols-2 gap-1 overflow-y-scroll hide-scrollbar-firefox hide-scrollbar-webkit">
      ...
      {products.length < 1 && !loading && !error && <span className="text-black">No Products</span>}
      {products.map((p) => {
        ...
        return (
          ...
          >
            <div className="flex flex-col justify-center h-full w-4/5">
              <span className="text-xl font-bold text-nowrap whitespace-nowrap text-ellipsis overflow-hidden w-full">
                {p.name}
              </span>
